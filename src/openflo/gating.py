            ...
            for ch in channels:
                ...
                if sub is None or len(sub) == 0:
                    ...
                    if vals.size == 0:
                        ...
                        # Geometric mean: fluorescence is approximately
                        # log-normal, so this — not the arithmetic mean — is
                        # the central tendency FlowJo reports and papers cite.
                        # Defined only on POSITIVE values; compensated data
                        # legitimately contains negatives, which are excluded
                        # rather than clamped (clamping would bias it upward).
                        pos = vals[vals > 0]
                        gmean = (float(np.exp(np.mean(np.log(pos))))
                                 if pos.size else float('nan'))
                        # Robust CV = 100 * 1.4826*MAD / median. The ordinary
                        # CV is dominated by the outliers real flow data always
                        # carries; this is the standard resistant form.
                        mad = float(np.median(np.abs(vals - med)))
                        rcv = (1.4826 * mad / med * 100.0

...
def _per_sample_inventory(reader):
    ...
    for sample_elem in reader.root.iter('Sample'):
        ...
        pops = []          # list of (pop_name, count, gate_dict, parent_name)

        def walk(pop_elem, parent_name):
            ...
            if g is not None:
                ...
                pops.append((pop_name, count, g, parent_name))  # noqa: B023
...
def _compare_one_sample(sample_name, fcs_path, pops, wsp_path=None):
    ...
    total = len(s.data)
    # Build a map from population name to its own gate dict (with an
    # auto id), and a parent-id linkage from the parent-name field, so
    # cumulative_gate_mask can walk up.
    pop_to_id   = {}
    gates_by_id = {}
    parents     = {}             # pop_name -> parent_pop_name ('' = root)
    for i, (pname, _cnt, gdict, parent_pname) in enumerate(pops, 1):
        gid = f'g{i}'
        pop_to_id[pname]   = gid
        gates_by_id[gid]   = dict(gdict)
        parents[pname]     = parent_pname
    # Resolve parent_id pointers into gid form (root parent stays None).
    for pname, gid in pop_to_id.items():
        ppname = parents.get(pname, '')
        gates_by_id[gid]['parent_id'] = pop_to_id.get(ppname) if ppname else None

    rows = []
    for pname, flowjo_count, _gdict, _parent in pops:
        gid = pop_to_id[pname]
        try:
            mask = cumulative_gate_mask(gates_by_id, gid, s.data)

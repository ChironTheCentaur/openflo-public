...
def multi_group_test(groups, paired=False):
    ...
    ``paired=True`` runs the Friedman test (which requires equal-length groups
    and >=3 groups). Non-finite values are dropped per group. For the paired
    case rows with any NaN are dropped listwise so the groups stay aligned.

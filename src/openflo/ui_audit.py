...
class AuditWindow(tk.Toplevel):
    ...
    def __init__(self, parent, audit_log):
        ...
        _am = ttk.Button(bar, text="Export Markdown…",
                         command=lambda: self._export('md'))
        _am.pack(side='right', padx=(0, 6), pady=4)
        auto_tip(_am,
                 "The audit trail as readable Markdown -- every action taken "
                 "in this session, in order. Suitable for a lab notebook or "
                 "a supplementary file.")
        _ac = ttk.Button(bar, text="Export CSV…",
                         command=lambda: self._export('csv'))
        _ac.pack(side='right', padx=(0, 4), pady=4)
        auto_tip(_ac, "The same trail as CSV, one row per action.")
        _aj = ttk.Button(bar, text="Export JSON…",
                         command=lambda: self._export('json'))
        _aj.pack(side='right', padx=(0, 4), pady=4)
        auto_tip(_aj,
                 "The trail as JSON, with full parameters -- the machine "
                 "-readable form for reproducing the analysis.")

        bar2 = ttk.Frame(self)
        bar2.pack(fill='x', side='top')
        ttk.Label(bar2, text="Compliance:", style='Muted.TLabel').pack(
            side='left', padx=6)
        _sg = ttk.Button(bar2, text="Sign & export record…",
                         command=self._sign_record)
        _sg.pack(side='left', padx=(0, 4), pady=2)
        auto_tip(_sg,
                 "Write the audit record with a cryptographic checksum, so "
                 "any later edit to the file can be detected. For work that "
                 "has to be shown to be unaltered.")
        _vf = ttk.Button(bar2, text="Verify record…",
                         command=self._verify_record)
        _vf.pack(side='left', pady=2)
        auto_tip(_vf,
                 "Check a signed record against its checksum and report "
                 "whether it still matches what was signed.")

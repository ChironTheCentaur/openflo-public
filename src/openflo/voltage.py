...
def main(argv=None):
    ...
    for ch in result['order']:
        ...
        for r in res['rows']:
            ...
            # SI / rCV are NaN when populations weren't separable — show 'n/a',
            # not a bare 'nan', so the reason reads clearly.
            si = f"{r['si']:.2f}" if np.isfinite(r['si']) else 'n/a'
            rcv = f"{r['rcv_neg']:.1f}" if np.isfinite(r['rcv_neg']) else 'n/a'
            log.info('%8s %7d %10s %10s %8d',
                     v, r['n_files'], si, rcv, r['n_events'])
            all_rows.append(r)
    ...
    if args.out:
        ...
        def _clean(row):
            # Match the stdout representation: empty voltage / non-separable
            # SI-rCV write 'n/a', not a blank cell vs a bare 'nan'.
            out = dict(row)
            if out.get('voltage') is None:
                out['voltage'] = 'n/a'
            for kk in ('si', 'rcv_neg'):
                if kk in out and not np.isfinite(out[kk]):
                    out[kk] = 'n/a'
            return out
        with open(args.out, 'w', newline='', encoding='utf-8') as fh:
            w = csv.DictWriter(fh, fieldnames=list(all_rows[0].keys()))
            w.writeheader()
            w.writerows([_clean(r) for r in all_rows])
        log.info('\nWrote %s', args.out)

...
_TRUTH_DEAD, _TRUTH_DEBRIS, _TRUTH_DOUBLET = -1, -2, -3


# Overloads so `return_labels` selects the return TYPE. Without them every
# existing caller sees a `DataFrame | tuple[...]` union and fails type checking.
@overload
def immunophenotyping_sample(
    n: int = ..., seed: int = ..., group: str = ..., batch_gain: float = ...,
    fmo: str | None = ..., return_labels: Literal[False] = ...,
) -> pd.DataFrame: ...
...
def immunophenotyping_sample(n=20_000, seed=0, group='ctrl', batch_gain=1.0,
    ...
    ``return_labels=True`` additionally returns a Series of the TRUE per-event
    population ('CD4 T', 'B cell', …, plus 'dead' / 'debris' / 'doublet') — the
    ground truth a clustering result can be scored against, rather than merely
    counted. Used by the response-control suite
    (``openflo-selftest --controls``). The default returns the DataFrame alone,
    exactly as before."""
    rng = np.random.default_rng(seed)
    ...
    # Carry the true per-event population through the shuffle as an extra
    # column, then strip it straight back off. Riding along in `body` is what
    # keeps the labels aligned WITHOUT touching the RNG stream: shuffling a 2-D
    # array is a row-wise Fisher-Yates, so the number of draws depends on the
    # row count alone — adding a column changes nothing the golden baseline can
    # see. (Permuting an index instead WOULD consume different randomness and
    # silently re-baseline every 1e-9 golden metric.)
    truth = np.concatenate([
        assign.astype(float),                       # live → its population idx
        np.full(n_dead, _TRUTH_DEAD, dtype=float),
        np.full(n_debris, _TRUTH_DEBRIS, dtype=float),
        np.full(n_doublet, _TRUTH_DOUBLET, dtype=float)])
    body = np.column_stack([body, truth])
    rng.shuffle(body)
    truth, body = body[:, -1], body[:, :-1]
    ...
    if not return_labels:
        return df
    lut = {i: nm_ for i, nm_ in enumerate(names)}
    lut.update({_TRUTH_DEAD: 'dead', _TRUTH_DEBRIS: 'debris',
                _TRUTH_DOUBLET: 'doublet'})
    labels = pd.Series([lut[int(c)] for c in truth], name='truth')
    return df, labels
...
DEFAULT_SPILL_LEAKS = {('APC-A', 'APC-Fire-A'): 0.18, ('PE-A', 'BV605-A'): 0.10,
                       ('FITC-A', 'BV510-A'): 0.08, ('BV510-A', 'FITC-A'): 0.05,
                       ('BV605-A', 'PE-A'): 0.06}


def make_compensation_controls(out_dir, n=8000, seed=700, leaks=None):
    ...
    ``leaks`` maps ``(source_detector, destination_detector)`` to a spillover
    coefficient, overriding :data:`DEFAULT_SPILL_LEAKS`. Pass ``{}`` for a
    spillover-FREE negative control, or a scaled dict to titrate — the
    response-control suite (``openflo-selftest --controls``) uses both to prove
    the estimator tracks its input rather than returning a fixed answer.
    """
    ...
    spill = np.eye(k)
    leaks = DEFAULT_SPILL_LEAKS if leaks is None else leaks

...
class SampleQCWindow(tk.Toplevel):
    ...
    def __init__(self, editor):
        ...
        self._names = []
        self._D = None
        # Same class as the ui_voltage._chan_lookup landmine: _compute assigns
        # _D then _xy, so an exception between them (mds_embed raising) would
        # leave _D set and _xy missing, and _draw reads both.
        self._xy = None
        ...
        _q1 = ttk.Button(bar, text="Export AnnData (.h5ad)…",
                         command=self._export_h5ad)
        _q1.pack(side='right', padx=(0, 6), pady=4)
        auto_tip(_q1,
                 "Write the samples as an .h5ad file for scanpy / Python "
                 "single-cell tools, carrying the per-sample summary shown "
                 "here.")
        _q2 = ttk.Button(bar, text="Distance CSV…", command=self._export_csv)
        _q2.pack(side='right', padx=(0, 4), pady=4)
        auto_tip(_q2,
                 "The sample-to-sample distance matrix behind this plot. Use "
                 "it to justify calling a sample an outlier rather than "
                 "eyeballing the map.")
        _q3 = ttk.Button(bar, text="Figure…", command=self._export_fig)
        _q3.pack(side='right', padx=(0, 4), pady=4)
        auto_tip(_q3, "Save this QC map as a figure (PNG/PDF/SVG).")
    ...
    def _markers(self):
        names = self._samples()
        # No samples checked → no shared markers. Guarded here rather than at
        # each caller: both _compute and _export_h5ad call this BEFORE their
        # own length check, so an empty selection used to raise IndexError on
        # names[0] below (inside an after() callback → silent traceback).
        if not names:
            return []
        shared = None
        for n in names:
    ...
    def _draw(self):
        ...
        fig = self._fig
        fig.clear()
        # Both, not just _D: _compute assigns _D then _xy, so an exception
        # between them (mds_embed raising) leaves _D set and _xy None.
        if self._D is None or self._xy is None:
            return

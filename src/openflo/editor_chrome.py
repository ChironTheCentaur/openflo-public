...
class ChromeMixin(EditorMixin):
    ...
    def _retheme_open_tk(self, pal):
        """After a live theme switch, recolour raw-tk widgets in this window and
        every already-open child window. The Tk option DB only themes widgets at
        *creation*, so open dialogs would otherwise keep their old backgrounds
        (ttk widgets restyle live on their own). Only greyscale/default backgrounds
        are touched, so intentional colours (gate swatches, etc.) are left alone."""
        def walk(w):
            try:
                bg = w.cget('background')            # ttk widgets raise → skipped
                if bg:
                    r, g, b = w.winfo_rgb(bg)        # 0..65535
                    spread = (max(r, g, b) - min(r, g, b)) / 65535.0
                    if spread < 0.06:                # a default grey/white, not a swatch
                        cls = w.winfo_class()
                        if cls in ('Text', 'Listbox'):
                            w.configure(bg=pal['panel'], fg=pal['fg'])
                        elif cls in ('Frame', 'Toplevel', 'Labelframe', 'Canvas'):
                            w.configure(bg=pal['bg'])
            except Exception:
                pass
            for c in getattr(w, 'winfo_children', list)():
                walk(c)
        try:
            for top in self._root().winfo_children():
                walk(top)
        except Exception:
            pass
    ...
    def _set_theme(self):
        ...
        self._retheme_open_tk(pal)      # recolour raw-tk widgets in open dialogs
    ...
    def _show_or_raise(self, key, factory):
        ...
        win = factory()
        reg[key] = win
        # Release the registry slot when the window closes, so a destroyed
        # Toplevel (with its Figure / cached DataFrame) is freed immediately
        # instead of being retained until the tool is reopened. Guard on
        # e.widget is win (the <Destroy> event bubbles from child widgets too)
        # and reg.get(key) is win (don't evict a newer window's slot).
        try:
            win.bind('<Destroy>', lambda e, k=key, w=win: (
                reg.pop(k, None) if (e.widget is w and reg.get(k) is w)
                else None), add='+')
        except Exception:
            pass
        return win

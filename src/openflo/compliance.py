...
def sign_manifest(record, signer, meaning, time):
    ...
    SCOPE — what a signature does and does not cover. Excluding ``signatures``
    from the hash is what makes CO-SIGNING work: Bob signing must not
    invalidate Alice's signature. The cost is that the signature LIST itself is
    not integrity-protected, so **removing** a signature is not detectable —
    ``verify_record`` will report the remaining ones as valid. Altering the
    manifest, the audit trail, the version or any signed file IS detected, as
    is altering a signature's own recorded hash.

    Closing that gap means chaining (each signature covering the manifest plus
    all prior signatures), which would invalidate every record signed to date —
    a deliberate migration, not a silent change. Documented here so the
    guarantee is not assumed to be wider than it is.
    """
...
def verify_record(record):
    ...
    for name, info in (record.get('files') or {}).items():
        ...
        if not want:
            continue
        # A file that was signed but is now missing/moved is a change the record
        # must surface — treat absence as a FAILED check (fail-closed), not a
        # silent skip. Otherwise deleting every signed file leaves files_ok empty
        # and all([]) == True would wrongly report the record as valid.
        files_ok[name] = bool(path) and os.path.isfile(path) and \
            sha256_file(path) == want

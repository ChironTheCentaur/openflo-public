...
def freeze_autoclean_gate(gate, df, channel_labels=None):
    ...
    sample. Debris → fixed ``min_fsc``; viability → resolved ``channel`` +
    fixed ``max_signal``. Methods without a single threshold (doublets, margin,
    flow-rate, drift) are left untouched (still per-sample). Non-autoclean
    gates are returned unchanged."""
    ...
    for m in g.get('methods') or []:
        ...
        if key == 'debris':
            thr = autoclean_debris_threshold(df, mp)
            if thr is not None:
                mp['min_fsc'] = float(thr)
        elif key == 'viability':
...
def transform_values(values, method='logicle', t=262144, m=4.5, w=0.5, a=0,
    ...
    if method == 'hyperlog':
        return transforms.hyperlog(v.reshape(-1, 1), channel_indices=[0],
                                   t=t, m=m, w=w, a=a).flatten()
    if method == 'asinh':
        return np.arcsinh(v / float(cofactor))
    if method == 'log':
    ...
    def extract_gates(self, *, sample_node=None):
        ...
        def parse_rect(rect_elem, parent_imp_id):
            ...
            if len(parsed) == 1:
                ...
                if hi is not None:
                    log.warning(
                        "[WspReader] Skipped max-only RectangleGate "
                        "on '%s'", ch)
                    return None
            elif len(parsed) == 2:
        ...
        def parse_quadrant(quad_elem, parent_imp_id):
            ...
            for label, x0, x1, y0, y1 in [
                    ('Q++ (x>, y>)',  xdiv,  big,  ydiv,  big),
                    ('Q+- (x>, y<)',  xdiv,  big, -big,  ydiv),
                    ('Q-+ (x<, y>)', -big,  xdiv,  ydiv,  big),
...
class AcquisitionQC:
    ...
    def _drift_bad_bins(self, bins, channels, n_bins, threshold):
        ...
        for b in range(n_bins):
            ...
            row = {'bin': b}
            for ch in channels:
                row[ch] = self.data.loc[mask, ch].median()
            records.append(row)
        self.bin_stats = pd.DataFrame(records).set_index('bin')
    ...
    def _flowrate_bad_bins(self, bins, n_bins, threshold):
        ...
        counts = np.array([int(np.count_nonzero(bins == b))
                           for b in range(n_bins)])
...
class FlowSample:
    ...
    def _apply_comp(self, matrix, channels):
        ...
        self.data[avail] = self.data[avail].values @ inv.T
    ...
    def cluster(self, channels=None, k=30, n_jobs=1, max_events=None,
        ...
        # Decide which backend to attempt first.
        try_gpu = False
        if use_gpu is not False and GPU_CLUSTERING_AVAILABLE:
        ...
        if sub_comm is None:
            ...
            try:
                with contextlib.chdir(_PHENOGRAPH_CACHE_DIR):
                    sub_comm, _, Q = phenograph.cluster(
                        X_cluster, k=k, n_jobs=n_jobs)
            except Exception as e:
                raise ClusteringError(

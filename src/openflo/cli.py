...
# don't crash under the Windows default cp1252 codepage. Done at import for
# module-level output, and AGAIN at the top of main() — an import-time guard
# only ever protects the stream that existed at import, so anything that
# replaces sys.stdout afterwards (an embedding host, a GUI redirect, a test
# harness) silently loses the protection.
...
def _process_sample_task(task):
    ...
    try:
        ...
        if method == 'leiden':
            combined.run_leiden(n_neighbors=task['k'],
                                resolution=task.get('resolution', 1.0),
                                max_events=mev or 200_000, random_state=seed)
            if 'leiden' in combined.data.columns:
                combined.data['cluster'] = combined.data['leiden']
        elif method == 'flowsom':
            combined.run_flowsom(n_metaclusters=task.get('n_metaclusters', 10),
                                 max_events=mev or 50_000, seed=seed)
            if 'flowsom_meta' in combined.data.columns:
                combined.data['cluster'] = combined.data['flowsom_meta']
        else:
            combined.cluster(k=task['k'], n_jobs=task['n_jobs'],
                             max_events=mev,
                             vram_admission_gb=task.get('vram_admission_gb', 1.0),
                             random_state=seed,
                             reproducible=bool(task.get('reproducible', False)))
        ...
        for g in groups:
            for name in g['samples']:
                ...
                tasks.append({'name': name, 'group': g['name'],
                              ...
                              'method': _cc.get('method', 'phenograph'),
                              'resolution': _cc.get('resolution', 1.0),
                              'n_metaclusters': _cc.get('n_metaclusters', 10),
                              'reproducible': _cc.get('reproducible', False),
...
def main() -> int:
    ...
    ap.add_argument('--reproducible', action='store_true',
                    help='Seed PhenoGraph clustering (via its Leiden backend) so '
                         'a re-run is bit-identical. Leiden / FlowSOM / embeddings '
                         'are already seeded; default PhenoGraph Louvain is not.')
    ...
    ap.add_argument('--gates',       default='',
                    help='JSON list of gate dicts, e.g. '
                         '\'[{"kind":"threshold","channel":"BV421-A","value":0.5}]\'')

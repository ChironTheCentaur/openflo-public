...
class ConsoleLogMixin:
    ...
    if TYPE_CHECKING:                       # provided by the composed editor
        ...
        def winfo_exists(self) -> int: ...          # tk.Misc
    ...
    def _drain_log(self):
        ...
        try:
            ...
            # Guard the self-reschedule: .after targets the shared root interp
            # and won't raise after this window is destroyed, so without the
            # winfo_exists check the 300 ms loop would fire forever on a dead
            # widget, pinning the whole editor (samples + figure) in memory.
            try:

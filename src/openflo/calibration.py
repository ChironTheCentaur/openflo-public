...
def absolute_count_per_uL(cell_events, bead_events, bead_concentration_per_uL):
    """Absolute cell concentration (events/µL) from counting beads of *known
    concentration* (e.g. a fixed-volume bead suspension run with the sample).

    ``cells/µL = (cell_events / bead_events) · bead_concentration_per_uL``
...
def absolute_count_from_known_beads(cell_events, bead_events, beads_added,
                                    sample_volume_uL):
    """Absolute cell concentration (cells/µL) from a *known number of beads
    added* to a *known sample volume* (the lyophilised-pellet variant).

    ``cells/µL = (cell_events / bead_events) · beads_added / sample_volume_uL``

    Raises ``ValueError`` if ``bead_events == 0`` or ``sample_volume_uL == 0``."""
    bead_events = float(bead_events)
    sample_volume_uL = float(sample_volume_uL)
    if bead_events == 0:
        raise ValueError("bead_events must be non-zero to compute a ratio")
    if sample_volume_uL == 0:
        raise ValueError("sample_volume_uL must be non-zero")
    return (float(cell_events) / bead_events) * float(beads_added) / sample_volume_uL

...
class CompareWspDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        _wsp_e = ttk.Entry(wrow, textvariable=self._wsp_var)
        _wsp_e.pack(side="left", fill="x", expand=True, padx=4)
        auto_tip(_wsp_e,
            "Path to the FlowJo .wsp workspace to compare against. OpenFlo "
            "reads its gates and re-applies them to the same FCS files, so "
            "the two tools' population counts can be compared directly.")
        _wsp_b = ttk.Button(wrow, text="Browse…", command=self._pick_wsp)
        _wsp_b.pack(side="left")
        auto_tip(_wsp_b, "Choose the FlowJo .wsp workspace file.")

        # FCS directory picker row.
        drow = ttk.Frame(top)
        drow.pack(fill="x", pady=2)
        ttk.Label(drow, text="FCS directory (optional):", width=24).pack(side="left")
        _dir_e = ttk.Entry(drow, textvariable=self._dir_var)
        _dir_e.pack(side="left", fill="x", expand=True, padx=4)
        auto_tip(_dir_e,
            "Folder holding the .fcs files the workspace refers to. The .wsp "
            "stores names, not data, so the files have to be found here.")
        _dir_b = ttk.Button(drow, text="Browse…", command=self._pick_dir)
        _dir_b.pack(side="left")
        auto_tip(_dir_b, "Choose the folder containing the .fcs files.")

        ttk.Label(
            top, style="Muted.TLabel", justify="left", wraplength=900,
        ...
        auto_tip(self._run_btn,
            "Re-apply the workspace's gates and report OpenFlo's count beside "
            "FlowJo's for every population, with the difference. Runs over "
            "every sample, so it can take a while on a large workspace.")
        ...
        auto_tip(self._export_btn,
            "Save the comparison table as CSV. Enabled once a run has "
            "produced results.")
        ...
        for cid, heading, width, anchor in _COLUMNS:
            ...
            self._tree.column(cid, width=width, anchor=anchor,  # type: ignore[arg-type]
                              stretch=True)
        # Flagged-row tints tracked to the active theme (deep tints + light ink
        # on dark, so rows never render as an unreadable light band in Midnight).
        for _tag, (_bg, _fg) in flag_tints().items():
            self._tree.tag_configure(_tag, background=_bg, foreground=_fg)
        ...
        self._summary = ttk.Label(self, text="", style="Muted.TLabel")

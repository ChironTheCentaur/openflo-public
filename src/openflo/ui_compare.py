...
class CompareWspDialog(tk.Toplevel):
    ...
    def _run(self):
        ...
        threading.Thread(
            target=self._worker, args=(wsp, fcs_dir or None), daemon=True).start()

    def _worker(self, wsp, fcs_dir):
        """Background: parse the workspace and compare every sample."""
        try:
            from .compare import (
                WspReader,
                _compare_one_sample,
            ...
            return
        self.after(0, lambda: self._on_done(rows, skipped))

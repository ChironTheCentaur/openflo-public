...
class GroupStatsWindow(tk.Toplevel):
    ...
    def __init__(self, editor):
        ...
        _cb = ttk.Combobox(bar, textvariable=self._ch, values=chans,
                           state='readonly', width=26)
        _cb.pack(side='left', padx=6)
        auto_tip(_cb, "Channel whose intensity is compared between groups.")
        _b = ttk.Button(bar, text="Compare groups", command=self._run)
        _b.pack(side='left', padx=6)
        auto_tip(_b,
                 "Test this channel for a difference between the sample "
                 "groups and report the result below.")
        self._txt = tk.Text(self, wrap='word')
        self._txt.pack(fill='both', expand=True, padx=10, pady=(0, 6))
        ttk.Label(
            self, style='Muted.TLabel', wraplength=620, justify='left',
    ...
    def _run(self):
        ...
        for name, s in ed._samples.items():
            ...
            vals = s.data[ch].to_numpy(dtype=float)
            vals = vals[np.isfinite(vals)]
            # Skip an all-NaN channel rather than appending a nan "replicate"
            # that would poison the group's median and the p-values.
            if vals.size:
                groups.setdefault(tr, []).append(float(np.median(vals)))

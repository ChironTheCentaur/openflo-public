...
class FlowSOMTreeWindow(tk.Toplevel):
    ...
    def __init__(self, editor, sample):
        ...
        _fe = ttk.Button(bar, text="Export figure…", command=self._export)
        _fe.pack(side='right', padx=6, pady=4)
        auto_tip(_fe,
                 "Save the FlowSOM minimum-spanning tree: nodes are "
                 "metaclusters, sized by population and coloured by marker "
                 "expression.")
    ...
    def _draw(self):
        ...
        ink = plot_ink(self)          # legible node/spoke/label ink on either canvas

...
class PopulationAnnotationWindow(tk.Toplevel):
    ...
    def __init__(self, editor, sample):
        ...
        _cc = ttk.Combobox(ctl, textvariable=self.col_var, width=14,
                           state='readonly', values=cols)
        _cc.pack(side='left', padx=(2, 8))
        auto_tip(_cc,
                 "Column holding the cluster or population labels to "
                 "annotate.")
        ttk.Label(ctl, text="MEM threshold:").pack(side='left')
        self.thr_var = tk.StringVar(value='2')
        _ts = ttk.Spinbox(ctl, from_=0, to=10, width=5,
                          textvariable=self.thr_var)
        _ts.pack(side='left', padx=(2, 8))
        auto_tip(_ts,
                 "Minimum |MEM| score for a marker to be listed in a "
                 "population's label. Higher gives short, confident names; "
                 "lower includes weaker markers and longer names.")
        _mc = ttk.Button(ctl, text="Compute MEM", command=self._compute)
        _mc.pack(side='left')
        auto_tip(_mc,
                 "Score every marker for every population by how strongly it "
                 "is enriched or depleted against the rest, producing a "
                 "phenotype label like CD3+ CD4+ CD8-.")
        _me = ttk.Button(ctl, text="Export MEM CSV…", command=self._export)
        _me.pack(side='right')
        auto_tip(_me,
                 "Save the full marker x population MEM score matrix, not "
                 "just the labels.")
        ...
        _ab = ttk.Button(ref, text="Assign names → populations",
                         command=self._apply)
        _ab.pack(side='left', pady=(4, 0))
        auto_tip(_ab,
                 "Rename the populations in the gate tree to these phenotype "
                 "labels. Renaming only -- no gate or event is changed, and "
                 "it can be undone.")
        self.status = ttk.Label(ref, text="", style='Muted.TLabel')

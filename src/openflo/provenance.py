...
def _iter_entries(audit_entries, skipped=None):
    """Yield ``(action, details)`` tuples from an AuditLog or list of dicts.

    ``skipped`` (optional list) collects a short reason for every entry that
    could not be read. The caller is the Methods-paragraph builder, whose
    output goes into a paper: an unreadable entry means a step the user
    performed is absent from their published description of what they did, so
    it must be reportable rather than dropped in silence.
    """
    ...
    try:
        ...
        if skipped is not None:
            skipped.append("audit trail is not iterable")
        return
    for e in entries:
        if not isinstance(e, dict):
            if skipped is not None:
                skipped.append(f"entry is {type(e).__name__}, not a record")
            continue
        ...
        if not isinstance(details, dict):
            if skipped is not None:
                skipped.append(
                    f"{action or 'entry'}: details is "
                    f"{type(details).__name__}, not a record")
            details = {}
...
def methods_paragraph(audit_entries, samples=None):
    ...
    text = " ".join(sentences)
    if unreadable:
        # Deliberately intrusive. This paragraph is meant to be pasted into a
        # manuscript, so an unreadable audit entry - a step the user performed
        # that is now missing from the description - must be impossible to
        # publish without noticing. It appears only when something IS
        # malformed, never in normal use.
        text += (f" [!] {len(unreadable)} audit entr"
                 f"{'y was' if len(unreadable) == 1 else 'ies were'} "
                 f"unreadable and {'is' if len(unreadable) == 1 else 'are'} "
                 f"NOT described above; this Methods text may be incomplete "
                 f"({unreadable[0]}"
                 f"{f'; +{len(unreadable) - 1} more' if len(unreadable) > 1 else ''}).")
    return text

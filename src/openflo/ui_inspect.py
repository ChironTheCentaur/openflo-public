...
class FcsInspectorDialog(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        _b = ttk.Button(top, text="Pick FCS…", command=self._pick)
        _b.pack(side="left")
        auto_tip(_b,
                 "Open an .fcs file and show its raw metadata — every $ "
                 "keyword, the channel table and the data type — without "
                 "loading it as a sample. Useful when a file behaves oddly.")

...
class SliderMixin(EditorMixin):
    ...
    def _commit_slider_to_gate(self, refresh_tree=True):
        ...
        ``refresh_tree=False`` skips the samples-and-gates tree rebuild. The
        drag handlers pass it because ``ttk.Scale(command=...)`` fires
        continuously (tens of times a second) and the rebuild is O(total tree
        rows) across ALL loaded samples — 8 ms at 20 samples x 30 gates. The
        tree is refreshed once on ``<ButtonRelease-1>`` instead (see
        ``_on_slider_release``), matching how the plot drag handlers defer to
        ``_on_release``. The gate itself still updates every tick, so the plot
        keeps tracking the slider."""
        self._update_slider_ui()
    ...
    def _on_slider_release(self, *_):
        """End of a slider drag: rebuild the gate tree once, so its rows show
        the final value. Bound to <ButtonRelease-1> on both scales."""
        if self._slider_channel:
            self._refresh_gate_list()
    ...
    def _on_slider_lo(self, *_):
        if self._slider_updating:
            return
        # Fires continuously while dragging -> skip the tree rebuild; it
        # happens once in _on_slider_release.
        self._commit_slider_to_gate(refresh_tree=False)

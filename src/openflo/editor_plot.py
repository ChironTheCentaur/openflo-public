...
class PlotMixin(EditorMixin):
    ...
    # point, so it scales with series x events: ~1.1 s for 8 samples x 50k
    # events, against ~16 ms for a fixed corner. Since it runs on every
    # replot, it dominated the profile. The 'cached' mode (the default) keeps
    # auto-placement but pays for it once: solve 'best' on the first draw for
    # a given plot shape, remember where it landed in AXES coordinates, and
    # reuse that until the plot's meaning changes.
    _LEGEND_FIXED_LOC = 'upper right'
    _LEGEND_CACHE_MAX = 64

    def _legend_mode(self):
        var = getattr(self, '_legend_placement', None)
        val = var.get() if var is not None else 'cached'
        return val if val in ('cached', 'best', 'fixed') else 'cached'

    def _legend_cache_key(self):
        """What must invalidate a remembered position: the axes' meaning and
        which samples are drawn (a different entry count needs a new box)."""
        try:
            samples = tuple(sorted(self._target_samples('enabled')))
        except Exception:                              # noqa: BLE001
            samples = ()
        return (self.mode_var.get(), self.x_combo.get(),
                self.y_combo.get(), samples)

    def _apply_legend(self, ax=None, **kw):
        """Draw the legend honouring the `legend_placement` preference.
        Drop-in for ``ax.legend(..., loc='best')``."""
        ax = self.ax if ax is None else ax
        mode = self._legend_mode()
        if mode == 'fixed':
            return ax.legend(loc=self._LEGEND_FIXED_LOC, **kw)
        if mode == 'best':
            return ax.legend(loc='best', **kw)
        cache = getattr(self, '_legend_pos_cache', None)
        if cache is None:
            cache = self._legend_pos_cache = {}
        pos = cache.get(self._legend_cache_key())
        if pos is not None:
            return ax.legend(loc=pos, **kw)
        leg = ax.legend(loc='best', **kw)
        # Resolved during the draw; read it back in _capture_legend_pos.
        self._legend_pending = (self._legend_cache_key(), ax, leg)
        return leg

    def _capture_legend_pos(self):
        """After the draw that resolved a 'best' placement, record where it
        landed (axes fraction) so later replots skip the search. Best-effort:
        any failure just means the next replot solves 'best' again."""
        pend = getattr(self, '_legend_pending', None)
        if not pend:
            return
        self._legend_pending = None
        key, ax, leg = pend
        try:
            ...
            if cache is None:
                cache = self._legend_pos_cache = {}
            if len(cache) >= self._LEGEND_CACHE_MAX:
                cache.clear()          # bounded; re-solving is cheap enough
            cache[key] = (float(x), float(y))
        except Exception:                              # noqa: BLE001
            pass

    def _get_df(self, name, x, y=None, for_hist=False, downsample=True,
                gate_parent='auto'):
        """Frame for plotting/fitting.

        ``gate_parent`` selects WHICH population comes back:
          ``'auto'`` (default) — the display behaviour below (the union of
              every enabled gate's chain, when gates are applied);
          ``None`` — the whole sample, ungated;
          a gate id — exactly that gate's cumulative population.
        The explicit forms exist for callers that must fit on the same
        population the result will be attached to; ``'auto'`` answers "what is
        on screen", which is a different question.
        """
        ...
        if gate_parent != 'auto':
            # Explicit population. `None` means the whole sample; a gate id
            # means that gate's cumulative chain — regardless of what is
            # currently displayed or which gates are toggled on.
            if gate_parent is not None and gate_parent in sample_gates:
                from .pipeline import cumulative_gate_mask
                m = cumulative_gate_mask(
                    sample_gates, gate_parent, df,
                    overrides=self._autoclean_overrides(name, df))
                df = df[np.asarray(m, dtype=bool)]
        elif self.apply_gates_var.get() and sample_gates:
            from .pipeline import cumulative_gate_mask
            overrides = self._autoclean_overrides(name, df)
            mask = np.zeros(len(df), dtype=bool)
            any_enabled = False
            # One cache for this call: every enabled gate otherwise re-walks
            # and re-evaluates its whole ancestor chain, and a polygon gate is
            # ~250x a threshold gate. `df`, `sample_gates` and `overrides` are
            # all fixed within this call, which is exactly the cache's
            # precondition — so it must NOT outlive the call.
            mask_cache = {}
    ...
    def _show_empty_overlay(self):
        ...
        if ov is None:
            ...
            ttk.Label(ov, text="…or drag & drop FCS files anywhere.",
                      style='Muted.TLabel').pack(pady=(12, 0))
            ttk.Label(ov, text="Resuming a larger session may take a moment "
                      "to load.", style='Muted.TLabel').pack(pady=(2, 0))
    ...
    def _replot(self):
        ...
        if getattr(self, '_clustering_busy', False):
            # A clustering/embedding worker is mutating sample DataFrames off the
            # Tk thread right now; reading them here would race (transient KeyError
            # / torn read as columns appear). Skip — _finish_clustering clears the
            # flag and schedules a fresh replot when the run completes.
            return
        self._hide_empty_overlay()
        ...
        if not samples:
            self.ax.text(0.5, 0.5, 'Select one or more samples on the left',
                         ...
                         fontsize=11,
                         color=current_palette().get('plot_fg', 'grey'))
            self.ax.set_xticks([]); self.ax.set_yticks([])
            self._apply_plot_theme()
            self.canvas.draw_idle()
            return
        ...
        self._apply_plot_theme()
        self.canvas.draw_idle()
        # A 'best' legend resolves during the draw — remember where it
        # landed so the next replot can reuse it (see _apply_legend).
        self._capture_legend_pos()
    ...
    def _draw_highlight_overlays(self, samples, x, y):
        ...
        if handles:
            self._apply_legend(fontsize=8, framealpha=0.85)
    ...
    def _overlay_removed_events(self, samples, x, y, mode):
        ...
        if handles:
            self._apply_legend(handles=handles, fontsize=8,
    ...
    def _plot_histogram(self, samples, x):
        ...
        # Say how many events the SCALE cannot place. A log axis cannot show a
        # non-positive value at all — on a compensated channel that is
        # routinely a third of the sample — and the curve looks exactly the
        # same whether it omitted none of them or all of them. Drawn on the
        # axes rather than the status line so it survives into an exported
        # panel. Deliberately not "outside the plotted range": the view is a
        # robust percentile, so that would fire on every plot and mean nothing.
        hist_scale = (self._channel_scale.get(x, self._default_scale_for(x))
                      if x else 'linear')
        hidden = sum(_pm.unplottable_count(arr, hist_scale)
                     for _name, arr in clean_series)
        total = sum(int(arr.size) for _name, arr in clean_series)
        if hidden and total:
            self.ax.text(
                0.99, 0.01,
                f'{hidden:,} of {total:,} events '
                f'({100.0 * hidden / total:.1f}%) are non-positive and '
    ...
    def _plot_dot(self, samples, x, y, color):
        ...
        if color == 'By sample':
            ...
            if len(samples) > 1:
                self._apply_legend(fontsize=8, markerscale=4,
                                   framealpha=0.85)

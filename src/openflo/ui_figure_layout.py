...
class FigureLayoutDialog(tk.Toplevel):
    ...
    def __init__(self, parent, n_samples, mode, default_pairs, on_apply):
        ...
        for lbl, val in layouts:
            _rb = ttk.Radiobutton(lf, text=lbl, value=val,
                                  variable=self.layout_var,
                                  command=self._sync_enabled)
            _rb.pack(anchor='w')
            auto_tip(_rb, f"Arrange the panels: {lbl.lower()}.")
        ...
        auto_tip(self.ncols_spin,
                 "Panels per row in the assembled figure. Rows are "
                 "filled left to right, so this sets the shape of "
                 "the grid.")

        self.gates_var = tk.BooleanVar(value=True)
        _dg = ttk.Checkbutton(body, text="Draw gates on panels",
                              variable=self.gates_var)
        auto_tip(_dg,
                 "Overlay each gate's outline and its percentage on the "
                 "panels -- the usual convention for a gating figure.")
        _dg.grid(
        ...
        _bb2 = ttk.Button(bb, text="Build", command=self._apply)
        _bb2.pack(side='right', padx=(0, 6))
        auto_tip(_bb2,
                 "Assemble the multi-panel figure in a new window, where it "
                 "can be exported at publication resolution.")

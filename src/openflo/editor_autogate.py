...
class AutoGateMixin(EditorMixin):
    ...
    def _run_auto_gate(self, opts):
        ...
        if name is None or name not in self._samples:
            self.status_var.set("Select a sample first.")
            return
        # Resolve the destination population ONCE, and fit on exactly that.
        # In Display=filter mode _get_df would otherwise hand back the union of
        # every ENABLED gate's chain, while _add_gate parents the result on the
        # SELECTED row — so a gate fit on lymphocytes could be attached at root
        # and then select monocytes and debris. Fit domain must equal applied
        # domain.
        parent = self._selected_gate_id()
        method = opts.get('method')
        if method == 'singlet':
            self._auto_gate_singlet(name, opts, parent)
        elif method == 'gmm':
            self._auto_gate_gmm(name, opts, parent)
        elif method == 'threshold':
            self._auto_gate_threshold(name, parent)
        self._schedule_replot(0)

    def _auto_gate_singlet(self, name, opts, parent=None):
        ...
        if not (area and height):
            ...
            return
  # downsample=False: fit on the FULL sample. The display
        # subsample is capped at the smallest loaded sample's size and
        # seeded with hash((name, x, y, cap)), which Python randomises
        # per process — so fitting on it made proposals depend on what
        # else was loaded AND differ on every app restart. The pipeline
        # routines subsample deliberately (gmm_ellipse_gates: seed=42,
        # max_events=20_000); the other two are cheap O(n) statistics.
        df = self._get_df(name, area, height, downsample=False,
                          gate_parent=parent)
        ...
        # `sample=` alone understates the change: with '-> all shown' on
        # (default) the gate lands on every displayed sample.
        self._audit('autogate.singlet', sample=name,
                    applied_to=self._gate_targets_for_new(),
                    area=area, height=height,
                    k=float(opts.get('k', 3.0)),
    ...
    def _auto_gate_gmm(self, name, opts, parent=None):
        ...
        if not x or not y:
            ...
            return
        df = self._get_df(name, x, y, downsample=False,
                          gate_parent=parent)
        ...
        for i, (gate, info) in enumerate(proposals, 1):
            ...
            self._add_gate_multi(gate, parent_id=parent,
                                 audit=False)
        ...
        note = (f" — {weak} overlap heavily (separation < 2); review those"
                if weak else "")
        self._audit('autogate.gmm', sample=name,
                    applied_to=self._gate_targets_for_new(), x=x, y=y,
    ...
    def _auto_gate_threshold(self, name, parent=None):
        from .pipeline import auto_threshold
        x = self._resolve_channel(self.x_combo.get())
        if not x:
            self.status_var.set("Pick an X channel first.")
            return
        df = self._get_df(name, x, downsample=False,
                          gate_parent=parent)
        ...
        self._add_gate_multi({'kind': 'threshold', 'channel': x,
                              'value': float(thr)},
                             parent_id=parent, audit=False)
        self._audit('autogate.threshold', sample=name,
                    applied_to=self._gate_targets_for_new(), channel=x,
                    value=float(thr))
        self.status_var.set(

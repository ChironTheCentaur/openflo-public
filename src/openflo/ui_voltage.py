...
class VoltageDialog(tk.Toplevel):
    ...
    def __init__(self, editor):
        ...
        # Paint the window dark up-front (belt-and-braces with the option DB) so
        # there's no white flash before the first figure draw under a dark theme.
        try:
        ...
        self._running = False
        self._canvas = None
        # Populated by _populate_channels; initialised here so _selected_channel
        # can never hit an AttributeError if it is reached before a folder is
        # picked, or after _populate_channels returns early on an unreadable FCS.
        self._chan_lookup = {}
    ...
    def _build_ui(self):
        ...
        top = ttk.Frame(outer)
        top.pack(fill='x')
        _fb = ttk.Button(top, text='Folder…', command=self._pick_folder)
        _fb.pack(side='left')
        auto_tip(_fb,
                 "Folder holding the voltage-titration series: one FCS per "
                 "PMT voltage, same sample throughout. The voltage is read "
        ...
        auto_tip(self._channel_box,
                 "Detector to titrate. Each detector has its own "
                 "optimal voltage, so this is run one channel at a "
                 "time.")
        ...
        auto_tip(self._run_btn,
                 "Compute the Stain Index at each voltage and plot "
                 "the curve. The optimum is where it plateaus -- "
                 "past that, more voltage adds spread without adding "
                 "separation.")
        ...
        ttk.Label(outer, textvariable=self._status_var, style='Muted.TLabel',
        ...
        auto_tip(self._export_btn,
                 "Save Stain Index against voltage as CSV, for the "
                 "instrument-QC record.")

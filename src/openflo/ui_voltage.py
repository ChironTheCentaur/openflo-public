...
class VoltageDialog(tk.Toplevel):
    ...
    def _run(self):
        ...
        def worker():
            try:
                result = VoltageTitration.analyze(paths, channels=[channel])
                self.after(0, lambda: self._on_done(result, channel))
    ...
    def _maybe_theme(self):
        """Apply the dark figure palette when the editor has it enabled."""
        try:
            if bool(self.editor._dark_figs.get()):
    ...
    def _export(self):
        ...
        try:
            dark = False
            try:
                dark = bool(self.editor._dark_figs.get())
            except Exception:
                dark = False
            background = 'Dark' if dark else 'White'

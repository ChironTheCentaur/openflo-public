...
class DnDMixin(EditorMixin):
    ...
    def _insert_sample_subtree(self, name, parent_iid):
        ...
        def insert_gate(gid, _name=name, _sg=sample_gates,
            ...
            gate_color = g.get('color') or current_palette()['fg']
    ...
    def _handle_checkbox_click(self, row_id):
        ...
        # A gate / auto-clean-method enabled toggle changes real gating output,
        # so snapshot for undo BEFORE mutating (display-only plot toggles don't
        # need it). Without this, Ctrl+Z can't revert an enable/disable.
        if any((p := self._parse_iid(r)) and p[0] in ('gate', 'method')
               for r in rows):
            self._checkpoint()

...
class GatingMixin(EditorMixin):
        ...
        if gate.get('kind') in ('threshold', 'interval'):
            ...
            for gid in list(self._gates):
                ...
                if (g.get('kind') in ('threshold', 'interval')
                    ...
                    # Nudging a 1-D gate should update ONLY its geometry — keep
                    # the existing colour, enabled state, custom name and expand
                    # state (else a slider re-commit silently re-enables a
                    # disabled gate / drops its name).
                    for _k in ('color', 'enabled', 'name', 'open'):
                        if _k in g:
                            gate[_k] = g[_k]
    ...
    def _add_gate_multi(self, gate, parent_id=None, audit=True):
        ...
        if template is None:
            return gid
        # Where the original landed, as a population PATH. Gate ids are
        # per-sample, so the id itself is meaningless on a target — but the
        # path ("Cells/Singlets") usually is, because gate trees are built
        # from the same panel.
        from .gating import population_path
        parent_path = (population_path(self._gates, template['parent_id'])
                       if template.get('parent_id') else None)
        ...
        for name in targets:
            ...
            g['id'] = new_id
            # Re-parent by PATH, not by id. Rooting unconditionally (the old
            # behaviour) meant that with a parent selected the active sample
            # got the gate nested while every other sample got it at root —
            # so one gate name measured a DIFFERENT population per sample,
            # silently invalidating any cross-sample frequency comparison.
            # Falling back to root is still correct when the target genuinely
            # lacks that population, but it is now reported rather than silent.
            tgt_gates = self._sample_gates[name]
            match = None
            if parent_path:
                for cand_id in tgt_gates:
                    if population_path(tgt_gates, cand_id) == parent_path:
                        match = cand_id
                        break
                if match is None:
        ...
        msg = f"Gate applied to {len(targets) + 1} samples."
        if orphaned:
            msg += (f"  ⚠ {len(orphaned)} lack '{parent_path}' — rooted there "
                    f"({', '.join(self._short_sample(n, 14) for n in orphaned[:3])}"
                    f"{'…' if len(orphaned) > 3 else ''}); they measure a "
    ...
    def _on_motion(self, event):
        ...
        if kind in ('v', 'h'):
            ...
            # No _refresh_gate_list() here: this fires on every raw mousemove,
            # and the rebuild is O(total tree rows) across ALL samples — 8 ms
            # at 20 samples x 30 gates, i.e. ~480 ms of Tk work per second of
            # dragging. Every other drag kind already relies on _on_release to
            # refresh once at end-of-drag (see the comment there); the line
            # gates were the lone exception. The moved line itself is redrawn
            # above, so the plot still tracks the cursor.
            self.canvas.draw_idle()

...
class SyntheticDialog(tk.Toplevel):
    ...
    def __init__(self, editor):
        ...
        auto_tip(self._type_cb,
                 "Which synthetic dataset to build. Each has known "
                 "ground truth, so it can be used to check a "
                 "workflow does what you expect before running real "
                 "samples. The parameters below change with the "
                 "choice.")
        ...
        _oe = ttk.Entry(frm, textvariable=self._out_var, width=34)
        _oe.grid(row=2, column=1, sticky='ew', pady=(4, 0))
        auto_tip(_oe, "Folder the generated .fcs files are written to.")
        _ob = ttk.Button(frm, text='Browse...', command=self._pick_dir)
        _ob.grid(row=2, column=2, sticky='ew', padx=(6, 0), pady=(4, 0))
        auto_tip(_ob, "Choose the output folder.")
        ...
        auto_tip(self._gen_btn,
                 "Write the dataset to the output folder. Files are "
                 "generated from a fixed seed, so the same settings "
                 "always reproduce the same data.")
    ...
    def _rebuild_fields(self):
        ...
        for r, (key, label, default) in enumerate(spec['fields']):
            ...
            _fe = ttk.Entry(self._fields_frame, textvariable=var, width=12)
            _fe.grid(row=r, column=1, sticky='w', padx=(6, 0), pady=2)
            auto_tip(_fe, f"Parameter '{key}' for this synthetic dataset.")
    ...
    def _generate(self):
        ...
        def _worker():
            ...
            return fn(out_dir, params)

        run_async(self, _worker,
                  on_done=lambda paths: self._on_done(paths, type_name),

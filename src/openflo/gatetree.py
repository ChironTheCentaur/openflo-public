...
# Structural ink for the free artists matplotlib's dark pass (_theme_figure_dark)
# doesn't recolour — connector lines, node labels and the empty-state text.
# Two neutral values so the diagram reads on a light OR a dark (Midnight) canvas;
# the caller passes `dark` from the editor's theme state. Kept as plain constants
# to preserve this module's GUI-/theme-free standalone contract.
_TEXT_INK = {'light': '#20242b', 'dark': '#e6e6e6'}
_EDGE_INK = {'light': '#555555', 'dark': '#8a8f98'}
...
def gate_tree_figure(
    ...
    ``dark`` picks the structural ink (connector lines, labels, empty-state
    text) so it stays legible when the figure is shown on a dark Midnight
    canvas; the gate box colours themselves are unchanged (data-viz).
    """
    ...
    ax.set_axis_off()
    _ink = _TEXT_INK['dark' if dark else 'light']
    _edge = _EDGE_INK['dark' if dark else 'light']
    ...
    if not positions:
        ax.text(0.5, 0.5, 'No gates', ha='center', va='center',
                transform=ax.transAxes, fontsize=11, color=_ink)
        return fig
    ...
    for node in _iter_nodes(roots):
        ...
        for child in node['children']:
            ...
            ax.plot([px, cx], [py - box_h / 2, cy + box_h / 2],
                    color=_edge, linewidth=1.0, zorder=1)

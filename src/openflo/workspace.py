...
DEFAULT_RUN_CFG = {
    ...
    'seed': 42,
    # PhenoGraph's default Louvain is time-seeded (non-reproducible). True →
    # cluster() uses the seeded Leiden backend so a re-run is bit-identical.
    'reproducible': False,
...
class PipelineWorkspaceView(ttk.Frame):
    ...
    def _build(self):
        ...
        bar = ttk.Frame(self)
        bar.grid(row=0, column=0, pady=(0, 3))   # no sticky → centred in col
        ...
        _se = ttk.Entry(prow2, textvariable=self._seed_var, width=5)
        ...
        crow.pack()
        _cc = ttk.Checkbutton(crow, text='Concatenate group samples',
...
class WorkspacePanel(ttk.Frame):
    ...
    def _launch_next(self):
        ...
        # of it blocking the UI. It is disk-bound, so a worker is the only fix.
        #

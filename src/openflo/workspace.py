...
def prepare_unit(editor, members: list, label: str, cfg: dict) -> dict:
    ...
    for m in members:
        ...
        frames.append(df)
        gtags.append(np.repeat(gtag, len(df)))
        stags.append(np.repeat(stag, len(df)))
        groups.add(gtag)
    ...
    data['__group__'] = np.concatenate(gtags) if gtags else []
    data['__sample__'] = np.concatenate(stags) if stags else []

...
class PipelineWorkspaceView(ttk.Frame):
    ...
    def _build(self):
        ...
        # Buttons split across TWO centred rows (group/setup · edit/IO) so the
        # full toolbar fits without overflowing — important when the panel is
        # popped out, where one long row used to demand half the screen width.
        bar = ttk.Frame(self)
        ...
        def _vsep(parent):
            ttk.Separator(parent, orient='vertical').pack(
                side='left', fill='y', padx=6, pady=1)

        _ed = self._editor

        def _btn(parent, text, width, cmd, tip, **pk):
            b = ttk.Button(parent, text=text, width=width, command=cmd)
            b.pack(side='left', **pk)
            _tip(_ed, b, tip)
            return b

        # Row 1 — build / configure run units.
        _btn(row1, "＋ Group", 8, self._new_group,
             "Create an empty group; drag samples / gate leaves into it. Each "
             "group is co-embedded as one run unit.")
        _btn(row1, "Group sel", 9, self._group_selected,
             "Bundle the selected items into a new group.", padx=(3, 0))
        _btn(row1, "Group params…", 12, self._group_params,
             "Override the clustering method / k / resolution for the selected "
             "group (otherwise it uses the run-bar settings).", padx=(3, 0))
        _vsep(row1)
        _btn(row1, "Set comp…", 9, self._set_comp_picker,
             "Assign compensation beads/matrix to the selected group "
             "(cascades to its members).")
        _btn(row1, "Set FMOs…", 9, self._set_fmo_picker,
             "Assign FMO control files to the selected group for threshold "
             "gating.", padx=(3, 0))
        _btn(row1, "Markers…", 8, self._pick_markers,
             "Choose which marker channels feed clustering / embeddings "
             "(default: auto — fluor channels minus height/width).",
             padx=(3, 0))
        # Row 2 — edit / recipes / batch.
        _btn(row2, "Clear", 6, self._clear, "Remove all items from the workspace.")
        _btn(row2, "Remove", 7, self._remove_selected,
             "Remove the selected item(s) from the workspace.", padx=(3, 0))
        _vsep(row2)
        _btn(row2, "Save recipe…", 11, self._save_recipe,
             "Save the run settings (method, k, resolution, embeddings, seed, "
             "max events) to a JSON file to reuse on other datasets.")
        _btn(row2, "Load recipe…", 11, self._load_recipe,
             "Load run settings from a saved recipe and apply them here.",
             padx=(3, 0))
        _vsep(row2)
        _btn(row2, "Batch folder…", 12, self._batch_folder,
             "Run the current settings over every .fcs in a folder, unattended "
             "(clusters + embeddings + per-event CSV per file).")
        ...
        # Method row: clustering algorithm + Leiden resolution.
        ...
        ttk.Label(mrow, text='res').pack(side='left')
        ...
        params.pack()                            # centred in the run bar
        _kl = ttk.Label(params, text='k'); _kl.pack(side='left')
        _ke = ttk.Entry(params, textvariable=self._k_var, width=4)
        _ke.pack(side='left', padx=(1, 6))
        ...
        _me = ttk.Entry(params, textvariable=self._max_var, width=7)
        _me.pack(side='left', padx=(1, 6))
        ...
        _se = ttk.Entry(params, textvariable=self._seed_var, width=5)
        _se.pack(side='left', padx=(1, 6))
        _tip(_ed, _se, "Random seed — fixes the subsample + embedding for "
             "reproducible results.")
        # Embeddings on their own row so the five options + Concat don't crowd
        # the numeric params.
        embrow = ttk.Frame(runbar)
        embrow.pack()
        ttk.Label(embrow, text='embed:').pack(side='left', padx=(0, 2))
        for _txt, _var, _tt in (
            ...
            _cb = ttk.Checkbutton(embrow, text=_txt, variable=_var)
            _cb.pack(side='left', padx=(0, 2))
            _tip(_ed, _cb, _tt)
        _cc = ttk.Checkbutton(embrow, text='Concat', variable=self._concat_var)
        _cc.pack(side='left', padx=(6, 0))
        _tip(_ed, _cc, "Concatenate a group's samples into ONE co-embedded run "
             "(off = embed each sample separately).")
        ...
        self.tree.column('#0', anchor='w', stretch=True, width=150, minwidth=120)
...
class WorkspacePanel(ttk.Frame):
    ...
    def _build(self):
        ...
        _imp = ttk.Button(bar, text="Import all", width=9,
                          command=self._import_all)
        _imp.pack(side='left', padx=(4, 0))
        _tip(_ed, _imp, "Add every loaded sample to the workspace at once.")
        ttk.Separator(bar, orient='vertical').pack(
            side='left', fill='y', padx=6, pady=1)
        _sv = ttk.Button(bar, text="Save…", width=7, command=self._save)
        _sv.pack(side='left')
        _tip(_ed, _sv, "Save the workspace layout (groups + comp/FMO "
             "assignments + run settings) to a file.")
        _ld = ttk.Button(bar, text="Load…", width=7, command=self._load)
        _ld.pack(side='left', padx=(4, 0))
        _tip(_ed, _ld, "Load a previously saved workspace layout.")
        ttk.Separator(bar, orient='vertical').pack(
            side='left', fill='y', padx=6, pady=1)
        _cl = ttk.Button(bar, text="✕ Close", width=8,
                         command=self._close_clicked)
        _cl.pack(side='left')
    ...
    def _run(self):
        ...
        def _unit_cfg(u):
            cfg = dict(base_cfg)
            ov = self.model.groups.get(u.get('gid'), {}).get('cfg') or {}

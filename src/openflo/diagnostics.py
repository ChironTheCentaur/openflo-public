...
def check_tk() -> dict:
    """Is Tk importable, and if not, what does this platform need?

    `tkinter` is the one requirement that cannot be expressed as a dependency:
    it is a CPython stdlib module backed by a system Tcl/Tk, so pip can neither
    install it nor detect its absence. On Windows and macOS it ships with the
    interpreter; on Debian/Ubuntu the python3 package deliberately omits it.

    Which means `pip install openflo` can succeed completely and then
    `openflo-gui` dies with an ImportError that does not say what to do. This
    check exists to say it.
    """
    try:
        import tkinter
    except Exception as exc:                                  # noqa: BLE001
        remedy = {
            'linux': 'sudo apt install python3-tk   (Debian/Ubuntu) or '
                     'sudo dnf install python3-tkinter   (Fedora/RHEL)',
            'darwin': 'brew install python-tk, or use the python.org installer',
            'win32': 'reinstall Python from python.org with the "tcl/tk" option '
                     'enabled',
        }.get(sys.platform, "install your platform's Tk package for Python")
        return {'available': False, 'version': None,
                'detail': f'{type(exc).__name__}: {exc}', 'remedy': remedy}
    return {'available': True, 'version': str(tkinter.TkVersion),
            'detail': '', 'remedy': ''}
...
def format_report(report: dict) -> str:
    ...
    # Tk gets its own line: it is not pip-installable, so a missing Tk is a
    # system-package problem the user has to be told how to solve.
    tk = report.get('tk') or {}
    if tk:
        if tk.get('available'):
            L.append("Graphical interface:")
            L.append(f"  [ OK ] Tk {tk['version']:<16} "
                     "required by openflo-gui")
        else:
            L.append("Graphical interface:")
            L.append(f"  [FAIL] Tk           not available  ({tk['detail']})")
            L.append(f"         openflo-gui cannot start. Fix: {tk['remedy']}")
            L.append("         The CLI (openflo-run) and the library API do "
                     "not need Tk.")
        L.append("")

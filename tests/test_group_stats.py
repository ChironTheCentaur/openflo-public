...
def test_volcano_significant_requires_both_p_and_effect():
    """'significant' = (p_adj<=alpha) AND (|effect|>=effect_cut). Isolate each
    condition — existing coverage only has significant-on-both or neither."""
    from openflo.stats import volcano_data
    res = [{'feature': 'both', 'effect': 2.0, 'p_adj': 0.001},     # both → sig
           {'feature': 'p_only', 'effect': 0.1, 'p_adj': 0.001},   # small effect
           {'feature': 'eff_only', 'effect': 3.0, 'p_adj': 0.5}]   # big p
    sig = {p['feature']: p['significant']
           for p in volcano_data(res, alpha=0.05, effect_cut=1.0)}
    assert sig == {'both': True, 'p_only': False, 'eff_only': False}


def test_compare_groups_reports_sample_sd():
    """Per-group 'sd' is the sample (ddof=1) standard deviation — never pinned
    before (only 'n' exact and 'median' approx were checked)."""
    res = compare_groups({'A': [10.0, 12.0], 'B': [1.0, 3.0]})
    a = res['groups']['A']
    assert abs(a['sd'] - 2.0 ** 0.5) < 1e-9          # sqrt(2), not population SD (1.0)
    assert abs(a['mean'] - 11.0) < 1e-9

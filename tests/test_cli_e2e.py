...
# `--help` is a sub-3 s wiring check on a normal install, so 30 s is plenty.
# But on a RAPIDS image, importing the CLI drags in cuDF/cuML (CUDA context +
# numba warm-up, 30-60 s) before argparse can print. That's import latency, not
# a wiring regression — so the bound is env-overridable: default CI stays strict
# at 30 s; the RAPIDS image sets OPENFLO_CLI_HELP_TIMEOUT to a larger value.
_HELP_TIMEOUT = int(os.environ.get('OPENFLO_CLI_HELP_TIMEOUT', '30'))
...
def test_cli_help_exits_zero():
    ...
    init)."""
    result = _run_cli(['--help'], timeout=_HELP_TIMEOUT)

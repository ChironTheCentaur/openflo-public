...
def test_session_writer_adds_portable_relink_fields(tmp_path):
    ...
    # rel_path is stored with forward slashes so a Windows-authored session
    # relinks on Linux/macOS/web (not a literal 'data\\a.fcs' filename there).
    assert by['A']['rel_path'] == os.path.relpath(
        str(a_fcs), str(tmp_path)).replace(os.sep, '/')
    assert '\\' not in by['A']['rel_path']

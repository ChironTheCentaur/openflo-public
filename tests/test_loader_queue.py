...
class _FakeEditor:
    ...
    def __init__(self):
        self._load_queue = gui.queue.PriorityQueue()
        self._load_seq = 0
        self._load_pool = []
        ...
        self._load_total = 0
        self._load_done = 0
        self._pool_size = lambda: 2          # fixed for a deterministic test
        self._lower_loader_priority = lambda: None
        # Concurrency probes.
        self._active = 0
        self._max_active = 0
        ...
        self._enqueue_load = GE._enqueue_load.__get__(self)
...
def test_pool_caps_concurrency_and_counts_exactly():
    ...
    n = 25
    for i in range(n):
        f._enqueue_load((f'sample{i}', f'/x/sample{i}.fcs'))
        f._load_total += 1
    ...
    for _ in range(gui._LOAD_POOL_SIZE):
        f._enqueue_load(None, priority=-1)
...
def test_done_counter_survives_failing_loads():
    ...
    n = 8
    for i in range(n):
        f._enqueue_load((f'bad{i}', f'/x/bad{i}.fcs'))
        f._load_total += 1
    _drain(f, n)
    assert f._load_done == n

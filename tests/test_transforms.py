...
def test_log_gives_no_coordinate_to_nonpositive_values():
    """A non-positive value has no logarithm.

    These used to clamp to 0.0, which on this scale means raw intensity 1.0 —
    so the dimmest events ranked ABOVE every genuinely positive event below
    1.0, and a third of a compensated channel piled onto one coordinate. NaN
    instead, matching the logicle/hyperlog branch, so `dropna` keeps them out
    of gates and medians. See tests/test_log_transform_nonpositive.py.
    """

...
def test_run_trimap_optional():
    import pytest
    pytest.importorskip("trimap")
    s = _stub(n=300)
    FlowSample.run_trimap(s, sample_n=10_000)
    assert 'TRIMAP1' in s.data.columns and 'TRIMAP2' in s.data.columns
    assert s.data[['TRIMAP1', 'TRIMAP2']].notna().all().all()


def test_run_pacmap_optional():
    import pytest
    pytest.importorskip("pacmap")
    s = _stub(n=300)
    FlowSample.run_pacmap(s, sample_n=10_000)
    assert 'PACMAP1' in s.data.columns and 'PACMAP2' in s.data.columns
    assert s.data[['PACMAP1', 'PACMAP2']].notna().all().all()

...
def _editor_stub():
    ...
    stub._stats_snapshot = types.MethodType(V._stats_snapshot, stub)
    stub._stats_rows_from_snapshot = types.MethodType(
        V._stats_rows_from_snapshot, stub)
    return stub
...
def test_stats_snapshot_path_matches_sync():
    """The backgrounded snapshot path (_stats_snapshot → _stats_rows_from_snapshot)
    produces the SAME rows + columns as the synchronous _collect_stats_rows."""
    stub = _editor_stub()
    sync_rows, sync_cols = V._collect_stats_rows(stub, WANT_ALL)
    snap = stub._stats_snapshot(WANT_ALL)
    async_rows, async_cols = stub._stats_rows_from_snapshot(snap, WANT_ALL)
    assert async_cols == sync_cols
...
def _cluster_editor(samples):
    ...
    for fn in ('_sample_cluster_ids', '_cluster_display_name', '_population_label',
               '_next_gate_id_for', '_import_clusters', '_import_population_column',
...
def test_sample_label_values_skips_unassigned_sentinel():
    ...
    # -1 (unassigned) is dropped by default…
    assert ed._sample_label_values('A', 'flowsom_meta') == [0, 1, 2]
    # …but include_sentinel keeps it (sorted last) so the importer can label it.
    assert ed._sample_label_values(
        'A', 'flowsom_meta', include_sentinel=True) == [0, 1, 2, -1]
...
def test_import_label_populations_creates_category_gates():
    ...
    # -1 (unassigned) is now KEPT and clearly labelled, not silently dropped.
    assert {g['value'] for g in cats} == {0, 1, 2, -1}
    assert all(g['channel'] == 'flowsom_meta' for g in cats)
    assert all(g['enabled'] is False for g in cats)
    noise = next(g for g in cats if g['value'] == -1)
    assert noise['name'] == 'Unassigned (noise)'          # explicit noise label

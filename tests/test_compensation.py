...
def test_singular_spillover_matrix_is_skipped_not_crash():
    """A singular spillover matrix (can't be inverted) must skip compensation
    with a warning, not raise LinAlgError and abort the load/run."""
    import pandas as pd
    chans = ['A', 'B']
    singular = np.array([[1.0, 1.0], [1.0, 1.0]])
    df = pd.DataFrame({'A': [10.0, 20.0], 'B': [5.0, 6.0]})
    s = fp.FlowSample.from_dataframe(df.copy(), name='x')
    s.manual_compensate(singular, chans)                # must not raise
    np.testing.assert_allclose(s.data[chans].to_numpy(), df.to_numpy())


def test_optimize_compensation_recovers_known_spillover(tmp_path):
    # The spillover ESTIMATOR (GUI "Optimize" button / public API) had no test.
    # B carries exactly 0.2x of A; C has NO A-dependence (constant) → slope 0.
    # (An independent RANDOM C is not a valid zero-check: over the narrow
    # top-percentile x-range its regression slope is noisy/large — expected
    # estimator behaviour, not spillover.)
    import pandas as pd

    from openflo.fcs_export import write_fcs
    rng = np.random.default_rng(0)
    a = rng.uniform(0.0, 10000.0, 6000)
    df = pd.DataFrame({'A': a, 'B': 0.2 * a, 'C': np.full(6000, 5000.0)})
    p = tmp_path / 'stainA.fcs'
    write_fcs(df, str(p))
    chans, m = fp.optimize_compensation(['A', 'B', 'C'], {'A': str(p)},
                                        positive_percentile=95.0)
    assert chans == ['A', 'B', 'C']
    assert m[0, 0] == 1.0
    assert abs(m[0, 1] - 0.2) < 0.02            # recovered A→B spillover slope
    assert m[0, 2] == 0.0                       # A→C = 0 (no dependence)
    # channels without a single-stain control keep their identity row
    assert list(m[1]) == [0.0, 1.0, 0.0]
    assert list(m[2]) == [0.0, 0.0, 1.0]

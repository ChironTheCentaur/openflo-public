...
def test_from_dict_mseq_fallback_resumes_from_max_suffix():
    # A legacy file lacking '_mseq' whose surviving items have gaps (M1,M2,M5
    # after deletions) must resume the counter from the MAX suffix (5) — the next
    # add is M6, NOT a re-mint of M3/M4/M5 that would overwrite an existing item.
    m = WorkspaceModel()
    for i in range(1, 6):
        m.add_item(_pl(f's{i}'))                    # M1..M5
    m.remove_item('M3')
    m.remove_item('M4')                             # gap → M1, M2, M5
    d = m.to_dict()
    d.pop('_mseq', None)                            # simulate a legacy file
    m2 = WorkspaceModel.from_dict(d)
    assert set(m2.loose) == {'M1', 'M2', 'M5'}
    new = m2.add_item(_pl('s6'))
    assert new == 'M6'                              # not a re-mint of M5
    assert set(m2.loose) == {'M1', 'M2', 'M5', 'M6'}
...
def test_default_run_cfg_is_independent():
    cfg = default_run_cfg()
    # Pinned deliberately: a run config key is part of the .flowsession
    # schema, so one must never appear by accident. 'fast_graph' was added
    # 2026-09-09 (prune the SNN graph to kNN pairs); it defaults to False, so
    # sessions written before it load unchanged.
    assert set(cfg) == {'method', 'resolution', 'n_metaclusters', 'k',
                        'max_events', 'seed', 'reproducible', 'channels',
                        'umap', 'tsne', 'phate', 'trimap', 'pacmap',
                        'concatenate', 'fast_graph'}
    cfg['k'] = 999

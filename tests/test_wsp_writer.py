...
def test_wsp_reader_rejects_dtd_entity_bomb(tmp_path):
    # A crafted .wsp with an internal DTD/entity (billion-laughs vector) must be
    # refused before parsing — stdlib etree has no expansion guard.
    p = tmp_path / 'bomb.wsp'
    p.write_text('<?xml version="1.0"?>\n'
                 '<!DOCTYPE lolz [<!ENTITY a "AAAAAAAAAA">\n'
                 '<!ENTITY b "&a;&a;&a;&a;&a;&a;&a;&a;&a;&a;">]>\n'
                 '<Workspace>&b;</Workspace>', encoding='utf-8')
...
def test_reader_to_writer_direct_round_trip(tmp_path):
    """A direct WspReader -> WspWriter round-trip (no GUI id-assignment in
    between) must not crash and must preserve gate kinds + parent structure.
    Reader gates carry only `_import_id`; the writer synthesises an `id`."""
    src, _ = _build_multisample_wsp(tmp_path)
    gates = fp.WspReader(src).extract_gates()           # gates have _import_id, no 'id'
    assert gates and all('id' not in g for g in gates)
    import collections
    kinds_before = collections.Counter(g.get('kind') for g in gates)
    roots_before = sum(1 for g in gates if g.get('parent_id') is None)

    w = fp.WspWriter(cytometer='direct-rt')
    w.add_sample('rt', fcs_path='', channels=[], gates=gates)  # must NOT raise
    out = tmp_path / 'direct_rt.wsp'
    w.write(str(out))

    gates2 = fp.WspReader(str(out)).extract_gates()
    kinds_after = collections.Counter(g.get('kind') for g in gates2)
    roots_after = sum(1 for g in gates2 if g.get('parent_id') is None)

...
def test_gui_module_imports():
    ...
    try:
        ...
        if 'display' in str(e).lower() or 'tcl' in str(e).lower():
            gui_unavailable(f"GUI cannot initialise without a display: {e}")
        raise
...
def test_autoclean_mask_cache_correct_and_hits():
    ...
    try:
        ...
        assert ref == again == cached  # cached path must equal the uncached truth
    finally:
        try:
            root.destroy()
        except Exception:
...
def test_autoclean_override_is_full_data_consistent():
    ...
    try:
        ...
        # A modest excursion inside the late window. Judged against the FULL
        # acquisition — which is broadly bimodal, so its across-bin spread is
        # large — it is unremarkable. Judged against the late subset alone,
        # which is tight, it is a clear drift. The two verdicts therefore
        # genuinely disagree, which is the whole point of the override.
        fsc[(time > 60) & (time < 65)] += 3e4
        ...
        assert not np.array_equal(ov, recomputed), (
            'the naive recompute agreed with the override, so this test no '
            'longer demonstrates that the override is needed')
    finally:
        try:
            root.destroy()
        except Exception:
...
def test_statistics_window_gate_targets():
    ...
    root, ed, gui = _editor_or_skip()
    # The stats refresh is backgrounded; run it inline here so the table is
    # populated synchronously for the assertions below.
    _saved_run_async = _at.run_async
    ...
    try:
        ...
        _at.run_async = _saved_run_async
        try:
            root.destroy()
        except Exception:
            pass
...
def test_error_report_tokenises_and_keeps_keymap_local(tmp_path, monkeypatch):
    ...
    os.environ.setdefault('MPLBACKEND', 'Agg')
    # The tokeniser + keymap helpers live in editor_lifecycle (the editor's
    # lifecycle / error-reporting mixin module).
    lifecycle = importlib.import_module('openflo.editor_lifecycle')
    keyfile = tmp_path / 'error_report.keys.json'
    monkeypatch.setattr(lifecycle, '_error_keymap_path', lambda: str(keyfile))

    secret = r"C:\Users\alice\study\Subject_007.fcs"
    text = f"load {secret} failed; retry {secret}"
    out = lifecycle._tokenise_for_report(text, ['Subject_007'])
...
def test_recent_sessions_roundtrip(tmp_path, monkeypatch):
    ...
    root, ed, _gui = _editor_or_skip()
    monkeypatch.setattr('openflo.prefs._prefs_path',
...
def test_window_geometry_validation():
    ...
    try:
        ...
        # _restore_geometry lives in editor_lifecycle, so patch read_prefs there.
        lifecycle = importlib.import_module('openflo.editor_lifecycle')
        import unittest.mock as m
        ed.winfo_screenwidth = lambda: 1920
        ed.winfo_screenheight = lambda: 1080
        with m.patch.object(lifecycle, 'read_prefs', lambda: {}):
            assert ed._restore_geometry('1500x840') == '1500x840'   # no pref
        with m.patch.object(lifecycle, 'read_prefs', lambda: {'geometry': 'garbage'}):
            assert ed._restore_geometry('1500x840') == '1500x840'   # unparseable
        with m.patch.object(lifecycle, 'read_prefs', lambda: {'geometry': '99999x99999+0+0'}):
            assert ed._restore_geometry('1500x840') == '1500x840'   # absurd size
        with m.patch.object(lifecycle, 'read_prefs', lambda: {'geometry': '1200x700+99999+99999'}):

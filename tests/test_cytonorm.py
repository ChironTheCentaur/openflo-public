...
def test_per_metacluster_resists_proportion_confounding():
    """CytoNorm's reason-for-being over a global quantile norm: two batches with
    the SAME population locations but DIFFERENT proportions. A global quantile map
    is confounded by the proportion mismatch and drags the low population upward;
    a per-metacluster fit aligns each population and leaves it in place. Every
    existing test uses equal proportions + uniform shifts, so a collapse-to-global
    bug (ignoring the metacluster label) would pass them all."""
    rng = np.random.default_rng(0)

    def pops(n_lo, n_hi):
        lo = np.column_stack([rng.normal(0.0, 0.3, n_lo) for _ in CHS])
        hi = np.column_stack([rng.normal(6.0, 0.3, n_hi) for _ in CHS])
        return pd.DataFrame(np.vstack([lo, hi]), columns=CHS)

    a = pops(2500, 2500)          # 50/50 reference
    b = pops(4000, 1000)          # 80/20 — same locations, different proportions

    def frac_high(df):            # fraction of events in the high population
        return float((df['M1'].to_numpy() > 3.0).mean())

    g = CytoNorm(CHS, n_metaclusters=1, grid=(6, 6), seed=1).fit({'A': a, 'B': b})
    m = CytoNorm(CHS, n_metaclusters=6, grid=(6, 6), seed=1).fit({'A': a, 'B': b})
    # B is truly ~20% high. A global quantile map forces A's 50/50 shape onto B
    # (pushing low-pop events up past 3 → ~50% high = invented biology); a
    # per-metacluster fit aligns each population and preserves B's ~20%.
    fh_meta = frac_high(m.apply(b, 'B'))
    fh_glob = frac_high(g.apply(b, 'B'))
    assert fh_meta < fh_glob - 0.1        # per-metacluster preserves proportions
    assert abs(fh_meta - 0.2) < 0.1       # ...near B's true 20% high


def test_qc_reports_wasserstein_before_after_magnitude():
    """qc returns per-channel mean Wasserstein batch→goal before vs after; on a
    real shift the 'after' collapses far below 'before' (only after<=before+eps
    was checked — a wrong goal/aggregation could hold that weak property)."""
    rng = np.random.default_rng(0)
    a = _bimodal(rng, 4000, 0.0, 1.0)
    b = _bimodal(rng, 4000, 0.8, 1.15)
    cn = CytoNorm(CHS, n_metaclusters=1, grid=(6, 6), seed=1).fit({'A': a, 'B': b})
    q = cn.qc({'A': a.to_numpy(), 'B': b.to_numpy()})
    for ch in CHS:
        assert q[ch]['before'] > 0.1               # a real batch effect exists
        assert q[ch]['after'] < q[ch]['before'] * 0.1   # correction removes most
...
def test_cytonorm_rejects_too_few_quantiles():
    """n_quantiles < 2 can't fit a spline; fail fast with a clear error instead
    of a cryptic PchipInterpolator crash at apply() time."""
    with pytest.raises(ValueError):
        CytoNorm(CHS, n_quantiles=1)


def test_events_missing_channel_raises_clear_error():
    """A batch DataFrame missing a normalized channel raises a clear ValueError,
    not a cryptic pandas KeyError deep in fit()/qc()."""
    cn = CytoNorm(['CD3', 'CD4'])
    with pytest.raises(ValueError):
        cn._events(pd.DataFrame({'CD3': [1.0, 2.0]}))     # missing CD4

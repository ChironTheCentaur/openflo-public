...
def test_friedman_perfect_concordance_is_significant_not_nan():
    """Every subject sharing the SAME non-constant treatment profile is the
    maximally-significant Friedman case, not a degenerate one — it must not be
    suppressed to NaN. Only genuine no-within-subject-variation is undefined."""
    # 3 treatments x 6 subjects; every subject ranks t3 > t2 > t1 identically.
    out = multi_group_test({'t1': [1.0] * 6, 't2': [2.0] * 6, 't3': [3.0] * 6},
                           paired=True)
    assert out['p'] == out['p']            # not NaN
    assert out['p'] < 0.05
    # genuinely degenerate (no across-treatment variation in any subject) → NaN
    flat = multi_group_test({'t1': [5.0] * 6, 't2': [5.0] * 6, 't3': [5.0] * 6},
                            paired=True)
    assert flat['p'] != flat['p']          # NaN


def test_cohens_d_magnitude_pinned():
    """Cohen's d MAGNITUDE (pooled ddof=1 SD), not just its sign — a wrong pooled
    variance (ddof=0 or mis-weighted) would still pass the sign-only tests."""
    d = effect_size([10, 11, 12, 13], [1, 2, 3, 4])['cohens_d']
    assert d == pytest.approx(6.9714, rel=1e-3)   # mean-diff 9 / pooled SD sqrt(5/3)

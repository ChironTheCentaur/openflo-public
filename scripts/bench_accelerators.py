...
The two accelerate DIFFERENT stages, so the script reports both:
  ...
  1. LOAD MATH  — per-sample compensation (spillover matmul) + transform
     ...
     CuPy can accelerate it; RAPIDS does NOT touch it.
       backends: numpy (CPU)  vs  cupy (GPU, NVIDIA)  vs  torch (GPU, portable:
                 NVIDIA/AMD/Intel/Apple/DirectML)
...
def bench_load_math(rows, channels, repeats):
    ...
    try:
        ...
        # end-to-end incl. host->device upload + device->host download
        def roundtrip():
    ...
    # GPU (PyTorch — portable: NVIDIA / AMD (ROCm) / Intel (XPU) / Apple (MPS) /
    # DirectML). This is the AMD/Intel path; runs even when CuPy is absent.
    try:
        import torch
    except Exception as exc:                       # noqa: BLE001
        print(f"  torch (GPU)                        — not available ({exc})")
    else:
        dev = None
        if torch.cuda.is_available():
            ...
            try:
                ...
                if _dml.is_available():
                    dev = _dml.device()
            except Exception:                      # noqa: BLE001
                dev = None
        if dev is None:
            print("  torch (GPU)                        — no non-CPU device found")
        else:
            dt = getattr(dev, "type", "dml")

            def twork(X, Sinv):
                Xc = X @ Sinv
                Xt = torch.asinh(Xc / 150.0)       # arcsinh
                _ = torch.isfinite(Xt).all(dim=1)
                _ = torch.quantile(Xt, 0.99, dim=0)
                return Xt
            tsync = torch.cuda.synchronize if dt == "cuda" else (lambda: None)
            Xt_dev = torch.as_tensor(X_host, device=dev)
            St_dev = torch.as_tensor(Sinv_host, device=dev)
            tsync()
            tg, _ = _time(lambda: twork(Xt_dev, St_dev), repeats, sync=tsync)
            print(_fmt(f"torch (GPU {dt}, compute only)", tg, base=cpu))

            def troundtrip():
                xd = torch.as_tensor(X_host, device=dev)
                out = twork(xd, St_dev)
                return out.detach().to("cpu").numpy()
            te2e, _ = _time(troundtrip, repeats, sync=tsync)
            print(_fmt(f"torch (GPU {dt}, incl. H<->D transfer)", te2e, base=cpu))

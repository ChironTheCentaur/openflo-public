...
Publishing was **deliberately deferred** (decided 2026-09-08 at v2.4.2). As of 2026-09-10 every item below is closed and the package builds and validates clean at 2.6.0; what remains is not a code task but two account actions on PyPI, listed under "Turning it on" at the end. The original decision: "let's
...
- [x] **~~Check the README renders on PyPI.~~** DONE (2026-09-10). `twine
      check` PASSES on both the wheel and the sdist, which covers the
      description rendering. The links were the real problem and needed
      fixing by hand:

      There are no images, so no broken image paths. There were **4 relative
      links** (`docs/algorithms.md`, `CONTRIBUTING.md`, `LICENSE.txt` twice),
      all now absolute against the public mirror; the shipped METADATA in the
      built wheel contains zero relative links, and all three targets were
      confirmed to exist on `openflo-public` rather than merely assumed.

      The worse find, while checking those: the install instructions said
      `git clone https://github.com/ChironTheCentaur/openflo.git`, which is
      **private**. The public mirror carries that line verbatim today, so the
      repository's own primary install instruction is a 404 for everyone who
      is not the author — and it would have shipped to PyPI unchanged. Now
      points at `openflo-public`. Nothing else in the tracked tree references
      the private repo.

## Worth deciding, not obviously required

- [x] **~~Decide what `pip install openflo` promises.~~** DECIDED
      (2026-09-10): the middle ground, written into the README. It still leads
      with the repo plus launcher — that is the path tested end to end on
      every commit and the one to use for the GUI — and now has a **From
      PyPI** section presenting pip as the route for `openflo-run`,
      `openflo-compare` and importing `openflo.pipeline` in a script or
      container. It says plainly that the GUI works from a pip install too,
      being the same code, but that you then own the environment it needs.

      The Tk note moved into the README alongside it, closing the "still TODO"
      left on that item: `tkinter` cannot come from PyPI, so a bare
      Debian/Ubuntu box installs cleanly and then fails at `openflo-gui` with
      an ImportError that does not name the fix. The README now gives the
      apt/dnf/brew command and points at `openflo-doctor`, and repeats that a
      Tk-less install is not unhealthy.
...
- [x] **~~Consider a `Development Status` bump.~~** DECIDED (2026-09-10):
      **stays `3 - Alpha`.**

      The stated trigger was "when the audit sweeps stop finding
      event-deleting defects". They have — the gate-partition audit came back
      clean, and adjacent gates provably claim every integer boundary event
      exactly once. But the sweeps have not stopped finding **fail-open
      gates**, which is the same signal one level out: this month alone, the
      public-mirror scrub gate would pass a forbidden token in a latin-1 file,
      and the GUI guard let 55 tests skip silently under a broken Tk. Both
      were checks that could not fail, in the machinery meant to catch exactly
      that.

      The second reason is plainer. Alpha versus Beta is largely a statement
      about API stability, and this release changes a default that changes
      results (`run_louvain` restarts 20 → 5). A package cannot claim settled
      behaviour in the same release that moves it.

      The natural trigger for Beta is first publication plus some external
      use — a version that survives contact with an installer who is not the
      author. Revisit one release after publishing.
...
Everything in this document is closed. What is left is not code — it is two
actions on accounts, and they must match the workflow exactly or the publish
step fails with `invalid-publisher`.

Verified 2026-09-10: the name **`openflo` is available** on PyPI (404 on
`/pypi/openflo/json`), the package builds clean at 2.6.0, and `twine check`
PASSES on both the wheel and the sdist.

**1. Add a Trusted Publisher on PyPI.** No token is stored anywhere; the
workflow authenticates by OIDC. PyPI allows a *pending* publisher for a project
that does not exist yet, which is the case here. Under Publishing settings, add:

| field | value |
|---|---|
| PyPI Project Name | `openflo` |
| Owner | `ChironTheCentaur` |
| Repository name | `openflo` |
| Workflow name | `release.yml` |
| Environment name | `pypi` |

Two of those are easy to get wrong. The repository is the **private** one,
`ChironTheCentaur/openflo` — that is where `release.yml` runs; `openflo-public`
is only a mirror and never publishes. And the environment name must be `pypi`,
because `release.yml` declares `environment: pypi`; leaving it blank on PyPI
while the workflow sets one is a mismatch and is rejected.

**2. Set the repository variable.** Settings → Secrets and variables → Actions
→ Variables: `PYPI_PUBLISH` = `true`. Until then the publish job is *skipped*
rather than failed, so tag pushes stay green and the build half still validates
the package on every tag.

**Then:** push a `v*` tag. `release.yml` builds the sdist and wheel, runs
`twine check`, and publishes. A tag pushed before step 2 is not wasted — it
exercises the whole pipeline except the upload, which is a cheap way to find a
packaging problem without spending a version number.

**One-way door.** A version number on PyPI can never be reused, even after a
delete. Publishing 2.6.0 means 2.6.0 is spent. Worth a `--repository testpypi`
dry run first if there is any doubt about the metadata.
